...
class ReturnsFrame(ttk.Frame):
    ...
    def load(self):
        ...
        for x in rows:
            price=fmt(x["unit_price_cents"],"") + (f" / pack x{x['qty_multiplier']:g}" if x["pricing_mode"]=="PACK" else "")
            self.t.insert("", "end",values=(x["id"],x["name_snapshot"],f"{x['qty']:g}",f"{x['returned']:g}",f"{float(x['qty'])-float(x['returned']):g}",price))
    def do_return(self):
