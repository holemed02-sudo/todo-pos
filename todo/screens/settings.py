...
class SettingsFrame(ttk.Frame):
    def __init__(self,master,app):
        ...
        ttk.Label(self,text=self.tr('Paramètres','الإعدادات'),font=("Segoe UI",22,"bold")).pack(anchor="w",pady=(0,12))
        ...
        ttk.Checkbutton(f,text="Demander un client avant encaissement",variable=self.require_client).grid(row=4,column=0,columnspan=2,sticky="w")
        ttk.Label(f,text=self.tr('Message bas du ticket','رسالة أسفل التذكرة')).grid(row=5,column=0,sticky="w",pady=5);ttk.Entry(f,textvariable=self.footer,width=34).grid(row=5,column=1,padx=8)
        ttk.Label(f,text=self.tr('Limite résultats recherche','حد نتائج البحث')).grid(row=6,column=0,sticky="w");ttk.Entry(f,textvariable=self.search_limit,width=10).grid(row=6,column=1,sticky="w",padx=8)
        ttk.Button(f,text="Enregistrer",command=self.save).grid(row=7,column=0,pady=8)
