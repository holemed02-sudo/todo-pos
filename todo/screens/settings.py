...
class SettingsFrame(ttk.Frame):
    def __init__(self,master,app):
        ...
        ttk.Label(f,text="Message bas du ticket").grid(row=4,column=0,sticky="w",pady=5);ttk.Entry(f,textvariable=self.footer,width=34).grid(row=4,column=1,padx=8)
        ttk.Label(f,text="Limite résultats recherche").grid(row=5,column=0,sticky="w");ttk.Entry(f,textvariable=self.search_limit,width=10).grid(row=5,column=1,sticky="w",padx=8)
        ttk.Button(f,text="Enregistrer",command=self.save).grid(row=6,column=0,pady=8)
    ...
    def save(self):
        ...
        set_setting('drawer_enabled','1' if self.drawer_enabled.get() else '0');set_setting('drawer_pin',self.drawer_pin.get());set_setting('block_insufficient_stock','1' if self.block_insufficient.get() else '0')
        try:
