        ...
        try:
            ...
            for offer in quantity_rows:
                offers_by_product.setdefault(offer["product_id"],[]).append(
                    f'{offer["min_qty"]:g}@{offer["unit_price_cents"]/100:.2f}@{offer["pricing_mode"]}'
                )
                ...
                try:
                    ...
                    if raw_offers:
                        for token in raw_offers.split('|'):
                            ...
                            if len(parts)!=3:raise ValueError(self.tr("offre quantité invalide","عرض الكمية غير صالح"))
                            min_qty=float(parts[0]);offer_price=to_cents(parts[1]);mode=parts[2].upper()
