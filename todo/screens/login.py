...
class LoginDialog(tk.Toplevel):
    def __init__(self,master,on_login):
        ...
        ttk.Button(f,text="Entrer",command=self.login).pack(fill="x",pady=12)
        e.focus_force()
    def login(self):
        with connect() as c:
        ...
        if not u or not verify_pin(self.pin.get(),u["pin_hash"]):
            messagebox.showerror("ToDo","Utilisateur/PIN incorrect.",parent=self);return
