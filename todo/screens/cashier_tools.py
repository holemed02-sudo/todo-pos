...
class Calculator(tk.Toplevel):
    def __init__(self,master):
        ...
        self.title('Calculatrice');self.transient(master.winfo_toplevel());self.grab_set()
        ...
        ttk.Button(self,text='Fermer',command=self.destroy).grid(row=7,column=0,columnspan=4,pady=8)
    ...
    def press(self,key):
        ...
        if key=='C':self.expression.set('')
            ...
            except (ValueError,SyntaxError,ArithmeticError,InvalidOperation):self.error.config(text='Calcul invalide ou division par zéro.')
...
class CashierLock(tk.Toplevel):
    def __init__(self,app):
        ...
        self.title('Caisse verrouillée / الصندوق مقفل')
        ...
        ttk.Label(self,text='PIN pour reprendre le ticket / الرمز للرجوع للبيع').pack(padx=20)
        ...
        ttk.Button(self,text='Déverrouiller / فتح',command=self.unlock).pack(pady=15)
        self.bind('<Return>',lambda e:self.unlock());entry.focus_set()

    def unlock(self):
        if time.monotonic()<self.blocked_until:
            self.error.config(text='Patientez 30 secondes.');return
        with connect() as conn:
            ...
            if not user or not verify_pin(self.pin.get(),user['pin_hash']):
                ...
                self.error.config(text='PIN incorrect');return
