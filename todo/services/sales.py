...
def complete_sale(session_id,user_id,cart,payment_method,paid_cents,discount_cents=0,held_id=None,client_id=None,payments=None,seller_id=None):
    ...
    with connect() as conn:
        ...
        for line in cart:
            ...
            allow_negative=conn.execute("SELECT value FROM settings WHERE key='allow_negative_stock'").fetchone()
            must_block=(block_stock and block_stock[0]=='1') or not (allow_negative and allow_negative[0]=='1')
            if must_block and not p['is_misc'] and qty>float(p['stock_qty']):
...
def hold_sale(user_id,cart,label='',discount_cents=0,held_id=None,client_id=None,seller_id=None):
    ...
    with connect() as conn:
        ...
        if client_id is not None and not conn.execute('SELECT id FROM clients WHERE id=? AND active=1',(client_id,)).fetchone():
            raise ValueError('Client introuvable.')
        if held_id is None:
            held_id=conn.execute('INSERT INTO held_sales(label,cashier_user_id,payload_json,discount_cents,client_id,seller_id) VALUES(?,?,?,?,?,?)',(label or 'Ticket en attente',user_id,payload,int(discount_cents),client_id,seller_id)).lastrowid
        else:
            cur=conn.execute('UPDATE held_sales SET label=?,payload_json=?,discount_cents=?,client_id=?,seller_id=? WHERE id=? AND cashier_user_id=?',(label or 'Ticket en attente',payload,int(discount_cents),client_id,seller_id,held_id,user_id))
...
def resume_held(held_id):
    with connect() as conn:
        ...
        return dict(cart=cart,discount_cents=row['discount_cents'],held_id=row['id'],client_id=row['client_id'],seller_id=row['seller_id'])
